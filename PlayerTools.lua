...
local PartRingSettings = {
        ...
        PullGain = 26,
        MinimumPullSpeed = 30,
        MaximumPullSpeed = 400,
        MaximumDynamicSpeed = 9000,
        MaximumAssemblyMass = 900,
        MaximumAssemblySize = 80,
        RescanInterval = 0.5,
        Parts = {},
...
do
        ...
        local function canRingPart(part, targetRoot)
                ...
                local maximumAssemblyMass = math.clamp(
                        tonumber(PartRingSettings.MaximumAssemblyMass) or 900,
                        1,
                        100000
                )
                local maximumAssemblySize = math.clamp(
                        tonumber(PartRingSettings.MaximumAssemblySize) or 80,
                        1,
                        10000
                )
        ...
        local function updatePartRing(deltaTime)
                ...
                local pullGain = math.clamp(
                        tonumber(PartRingSettings.PullGain) or 26,
                        1,
                        100
                )
                ...
                local baseMaximumPullSpeed = math.clamp(
                        tonumber(PartRingSettings.MaximumPullSpeed) or 400,
                        minimumPullSpeed,
                        2500
                )
                local maximumDynamicSpeed = math.clamp(
                        tonumber(PartRingSettings.MaximumDynamicSpeed) or 9000,
...
task.spawn(bindMovementCharacter, LocalPlayer.Character)
...
local Window = Reborn:CreateWindow({
        ...
        Theme = {
                ...
                AccentHover = Color3.fromRGB(88, 152, 255)
        }
})
...
ClickTeleportSection:Paragraph({
        Text = "Left-click anywhere to teleport your character there."
})
...
PlatformSection:Paragraph({
        Text = "Controls: E raises, Q lowers, Shift moves toward the camera."
})
...
FlySection:Paragraph({
        Text = "Controls: WASD to move, Space to rise, Ctrl to descend."
})
...
AimlockSection:Paragraph({
        Text = "Hold right mouse button over a player to lock the camera onto them."
})
...
VehicleSection:Paragraph({
        Text = "Works while seated in any seat or vehicle. Press E to speed boost."
})
...
VehicleJumpSection:Paragraph({
        Text = "Press J while seated to jump the vehicle."
})
...
VehicleSpeedBoostSection:Paragraph({
        Text = "Press E while seated to launch the vehicle forward."
})
...
VehicleTeleportSection:Paragraph({
        Text = "Left-click anywhere to teleport the vehicle you are sitting in."
})
...
PartRingSection:Paragraph({
        Text = "Pulls nearby loose physics parts into a spinning shield ring around the target. Press K to hide or show this window."
})
...
do
local ControlsSection = FunTab:Section("Controls")

ControlsSection:Paragraph({
        Text = "K: hide or show this window. Right mouse: aimlock. E: platform up or vehicle boost. Q: platform down. Shift: platform forward. J: vehicle jump."
})
end
...
track(Mouse.Button1Down:Connect(function()
        ...
        if ok and objects and Window and Window.Gui then
                for _, object in ipairs(objects) do
                        if object:IsDescendantOf(Window.Gui) then
                                overWindow = true
                                break
                        end
                end
